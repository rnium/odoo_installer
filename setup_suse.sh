...
install_system_packages() {
    ...
    sudo zypper install -y python3-pip python3-virtualenv python3-devel
    # Install libxml and image libraries
    sudo zypper install -y libxml2-devel libxslt-devel libjpeg8-devel zlib-devel freetype2-devel tiff-devel libwebp-devel
    # Install LDAP development libraries
    sudo zypper install -y cyrus-sasl-devel libssl-devel openldap2-devel
    # Install PostgreSQL dev and server packages
    sudo zypper install -y postgresql postgresql-server postgresql-server-devel libpq5
    # Install build tools
    sudo zypper install -y gcc g++ make
...
clone_repo() {
    ...
    sudo chown -R $owner:$owner $workdir
}
...
install_requirements() {
    ...
    # Switch to the owner user for pip installation
    sudo -u $owner bash -c "
...
create_conf_file() {
    ...
    sudo chown $owner:$owner $conf_file
}
